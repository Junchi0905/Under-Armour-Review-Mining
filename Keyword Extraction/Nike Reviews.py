...
for file in os.scandir('Nike'):
    reviews=pd.read_csv(file)
    reviews.drop(['num_reviews','review_data','loc_type','avg_review','user_href','ID'],axis=1,inplace=True)
    ...
    for sentence in text:
        for i in keywords:
            if i in sentence:
                ...
                sentiment = sentiment.append({'Name': reviews['name_of_loc'][0],
                                  'Review':sentence,
                                  'Sentiment Score':ss['compound'],
                                    "Review Number":len(reviews),
                                    'lat':reviews['lat'][0],
                                    'long':reviews['long'][0]     
                                             }, ignore_index=True)
...
sentiment_gb=sentiment.groupby(['Name']).mean()
sentiment_gb
...
sentiment_gb.to_csv('Sentiment Nike.csv',index=True)
...
ldamodel = Lda(doc_term_matrix, num_topics=5, id2word = dictionary_review, passes=50)
